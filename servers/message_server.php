...
class message_server{
	...
	public function set_message_is_red($id){
		$id+=0;
		return $this->system->db()->do_SQL('UPDATE `'.self::table.'` SET `type`=`type`|1 WHERE `mid`='.$id);
	}
	public function send_message($sender,$body,$geter){
		$this->system->db()->u_do_SQL('INSERT INTO `'.self::table.'`(`sender`,`time`,`body`,`geter`) VALUES(?,?,?,?)',array($sender,time(),htmlentities($body,ENT_NOQUOTES,'utf-8'),$geter));
		//
	}
	...
	public function delete($mid,$t){
		$mid+=0;
		$this->system->db()->do_SQL('UPDATE `'.self::table.'` SET `type`=`type`|'.($t=='s'?4:2).' WHERE `mid`='.$mid);
	}
